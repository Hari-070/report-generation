...
export default function AdminPage() {
  ...
  const handleMigrate = async () => {
    if (
      !confirm(
        "This will assign all untagged client records to your account. Continue?",
      )
    )
      return;
    setMigrating(true);
    try {
      ...
      const data = await res.json();
      setMigrateResult(`✓ ${data.message} — ${data.updated} records updated`);
    } catch {
      ...
      >
        <div>
          ...
          <div style={{ color: "rgba(255,255,255,0.3)", fontSize: "11px" }}>
            Assign all untagged client records to your account
          </div>
          {migrateResult && (
            <div
              style={{ color: "#22c55e", fontSize: "11px", marginTop: "4px" }}
            >
              {migrateResult}
            </div>
          )}
      ...
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "48px 24px" }}>
        ...
        {searched && (
          <div
            style={{
              display: "grid",
              gridTemplateColumns: selected ? "340px 1fr" : "1fr",
              gap: "20px",
            }}
          >
            {/* Results list */}
                    ...
                    >
                      ...
                      >
                        ...
                        <div style={{ textAlign: "right" }}>
                          ...
                          >
                            {r.history?.length || 1} scan
                            {(r.history?.length || 1) > 1 ? "s" : ""}
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                >
                  ...
                  <div
                    style={{
                      fontSize: "36px",
                      fontWeight: "bold",
                      color: scoreColor(selected.scoreLabel),
                      textAlign: "right",
                    }}
                  >
                    {selected.score}
                          ...
                          >
                            <div
                              style={{
                                fontSize: "22px",
                                fontWeight: "bold",
                                color: scoreColor(scan.scoreLabel),
                              }}
                            >
                              {scan.score}
                            </div>
                            <div
                              style={{
                                ...
                                padding: "2px 8px",
                                borderRadius: "20px",
                                background: scoreColor(scan.scoreLabel) + "20",
                                color: scoreColor(scan.scoreLabel),
                              }}
                            >
                              {scan.scoreLabel}
